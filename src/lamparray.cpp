...
{
    std::string name = "LampArray sample";
...
void LampArray_::UpdateRequestLampFromLampAttributesRequestReport(uint8_t* data, uint16_t length) noexcept
{
    ...
    {
        log_i("Setting m_lastLampIdRequested to %d", report.LampId);
        m_lastLampIdRequested = report.LampId;
    }
    else
    {
        log_i("Range error: setting m_lastLampIdRequested to 0");
        m_lastLampIdRequested = 0;
    }
}
...
void LampArray_::onRead(BLECharacteristic* p)
{
  ...
  {
    const LampAttributesResponseReport *pReport = &s_lampAttributesReports[m_lastLampIdRequested];
    p->setValue((uint8_t *)pReport, sizeof(LampAttributesResponseReport)); 
    log_i("LampAttributesResponseReport size=%d, lampID=%d", sizeof(LampAttributesResponseReport), m_lastLampIdRequested);
...
/*
 *  Callback from BLE
 *  No great way as of now to do data validation on the data as helper library doesn't expose length of data
 */
void LampArray_::onWrite(BLECharacteristic* p)
{
  ...
  {
      Serial.println("LampMultiUpdateReport");
      UpdateLampStateCacheFromMultiUpdateReport(p->getData(), sizeof(LampMultiUpdateReport) );
  }
  else if (p == m_bleLampRangeUpdateReport)
  {
      Serial.println("LampRangeUpdateReport");
      UpdateLampStateCacheFromRangeUpdateReport(p->getData(), sizeof(LampRangeUpdateReport) );
  }
  else if (p == m_bleLampArrayControlReport)
  {
      Serial.println("LampArrayControlReport");
      UpdateCachedUseDefaultEffect(p->getData(),  sizeof(LampArrayControlReport) );    
      
  }
  else if (p == m_bleLampAttributesRequestReport)
  {
      Serial.println("LampAttributesRequestReport");
      UpdateRequestLampFromLampAttributesRequestReport(p->getData(), sizeof(LampAttributesRequestReport) );
  }
}
